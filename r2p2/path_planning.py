        ...
        if angle in found:  # There was already a node in that angle
            if u.los_raycasting(node.point, n.point, u.npdata) == (-1, -1):  # There is line of sight
                ...
                cur_dist = np.linalg.norm(
                    (node.point[0] - found[angle].point[0], node.point[1] - found[angle].point[1]))
...
def generate_waypoints_list(algo, start, finish, grid, heur=aux):
    """
...
def generate_waypoints_list_mesh(algo, start, finish, mesh, heur=aux):
    """
        Generates an ordered list of waypoints for a controller to iterate over.
        Inputs:
            - algo: string representing the path planning algorithm to be used.
...
def run_path_planning(grid_size, algo='A*', start=(1, 1), finish=(2, 2), heur=aux, show_grid=True):
    """
...
def run_path_planning_mesh(mesh_points, algo='A* mesh', start=(1, 1), finish=(2, 2), heur=aux):
    """

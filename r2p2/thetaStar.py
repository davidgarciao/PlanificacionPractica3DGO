...
def tStar(start, goal, grid, heur=aux):
    """
        ...
        for node in children(current,grid):
            ...
            if node in openset:
                ...
                if LineOfSight(current.parent, node, grid):
                    #Check if we beat the G parent score
...
def LineOfSight(nodo0, nodo1, grid):
    x0, y0 = nodo0.grid_point
    x1, y1 = nodo1.grid_point
    dy = y1 - y0
    dx = x1 - x0
    f = 0
    if dy < 0:
    ...
    if dx >= dy:
        while x0 != x1:
            f = f + dy
            if f >= dx:
                if check(x0 + int((sx - 1)/2), y0 + int((sy -1)/2), grid):
                    return False
                y0 = y0 + sy
                f = f - dx
            if f != 0 and check(x0 + int((sx - 1)/2), y0 + int((sy -1)/2), grid):
                return False
            if dy == 0 and check(x0 + int((sx - 1)/2), y0, grid) and check(x0 + int((sx - 1)/2), y0 - 1, grid):
                return False
        ...
        while y0 != y1:
            ...
            if f != 0 and check(x0 + int((sx - 1)/2), y0 + int((sy -1)/2), grid):
                return False
            if dx == 0 and check(x0, y0 + int((sx - 1)/2), grid) and check(x0 - 1, y0 + int((sx - 1)/2), grid):
                return False
...
def check(x,y,grid):
    return grid[x][y].value>=5

pp.register_search_method('theta*', tStar)